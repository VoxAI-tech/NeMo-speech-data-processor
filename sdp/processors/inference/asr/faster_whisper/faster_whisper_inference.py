...
class InferenceConfig:
    ...
    IMPORTANT: Updated settings to properly return multiple segments per audio:
    - without_timestamps: False (was True) - enables proper segmentation
    - word_timestamps: False (unchanged) - word-level timing not needed
    This change fixes the issue where all audios returned only a single segment.
    """
    ...
    without_timestamps: bool = False  # Changed to False to enable proper segmentation
    max_initial_timestamp: float = 1.0
    word_timestamps: bool = False  # Keep as False - word timestamps not needed
