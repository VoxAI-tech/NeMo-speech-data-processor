#!/bin/bash

# German Drive-thru Pipeline with Qwen LLM Menu Correction
# Full dataset processing version
# Uses vLLM library directly - no server needed

set -e

# Configuration
DATA_DIR="/data/razhan/speech-pipeline/audio/audiobox"
BRAND="bk"  # Burger King
COUNTRY="de"  # Germany
CUTOFF_DATE=$(date +%Y%m%d)  # Today's date
OUTPUT_DIR="outputs/${BRAND}-${COUNTRY}-${CUTOFF_DATE}-qwen"
SDP_DIR="$(pwd)"

echo "=========================================="
echo "German Pipeline - Qwen LLM Version (FULL DATASET)"
echo "=========================================="
echo "Model: Whisper large-v3"
echo "Correction: Qwen3-8B (text-based via vLLM)"
echo "Channel Normalization: ENABLED (25% boost for quieter channel)"
echo "Data: ${DATA_DIR}"
echo "Output: ${OUTPUT_DIR}"
echo "Processing: FULL DATASET"
echo "=========================================="
echo ""
echo "WARNING: This will process the ENTIRE dataset"
echo "Estimated time: 6-8 hours (depending on data size)"
echo "Estimated disk space needed: ~10GB"
echo ""

# Check if output directory already has data
if [ -f "${OUTPUT_DIR}/de/manifest_32.json" ]; then
    echo "WARNING: Output directory already contains processed data!"
    echo "File: ${OUTPUT_DIR}/de/manifest_32.json exists"
    read -p "Do you want to overwrite? (y/N): " -n 1 -r
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Aborting to preserve existing data."
        exit 1
    fi
    echo "Removing existing output directory..."
    rm -rf ${OUTPUT_DIR}
fi

# Create output directory
mkdir -p ${OUTPUT_DIR}

# Create a simple progress monitor script
cat > ${OUTPUT_DIR}/monitor_progress.sh << 'EOF'
#!/bin/bash
# Monitor pipeline progress by checking manifest files
OUTPUT_DIR="$1"
while true; do
    clear
    echo "Pipeline Progress Monitor"
    echo "========================="
    echo ""
    for i in {00..32}; do
        MANIFEST="${OUTPUT_DIR}/de/manifest_${i}.json"
        if [ -f "$MANIFEST" ]; then
            COUNT=$(grep -c '"audio_filepath"' "$MANIFEST" 2>/dev/null || echo "0")
            echo "✓ Stage $i: $COUNT entries"
        else
            echo "  Stage $i: pending..."
            break
        fi
    done
    echo ""
    echo "Last update: $(date '+%H:%M:%S')"
    echo "Press Ctrl+C to stop monitoring"
    sleep 10
done
EOF
chmod +x ${OUTPUT_DIR}/monitor_progress.sh

echo "TIP: To monitor progress in another terminal, run:"
echo "  ${OUTPUT_DIR}/monitor_progress.sh ${OUTPUT_DIR}"
echo ""

# Log start time
START_TIME=$(date +"%Y-%m-%d %H:%M:%S")
START_EPOCH=$(date +%s)
echo "Start time: ${START_TIME}"
echo ""

# Function to handle interruption
cleanup() {
    echo ""
    echo "=========================================="
    echo "Pipeline interrupted!"
    echo "=========================================="
    END_TIME=$(date +"%Y-%m-%d %H:%M:%S")
    END_EPOCH=$(date +%s)
    DURATION=$((END_EPOCH - START_EPOCH))
    HOURS=$((DURATION / 3600))
    MINUTES=$(( (DURATION % 3600) / 60 ))
    echo "Start time: ${START_TIME}"
    echo "End time: ${END_TIME}"
    echo "Duration: ${HOURS}h ${MINUTES}m"
    echo ""
    echo "Partial results saved to: ${OUTPUT_DIR}"
    echo "Check the last manifest file to see progress."
    exit 1
}

# Set up trap for Ctrl+C
trap cleanup INT

# Run the pipeline WITHOUT max_samples parameter to process all data
echo "Starting Qwen-based pipeline processing on FULL dataset..."
echo "vLLM will be initialized directly by the processor - no server needed."
echo "This will take several hours..."
echo ""

uv run python main.py \
    --config-path dataset_configs/vox_pipeline/ \
    --config-name config_de_qwen.yaml \
    data_dir="${DATA_DIR}" \
    output_dir="${OUTPUT_DIR}" \
    sdp_dir="${SDP_DIR}" \
    max_samples=-1 \
    params.save_disk_space=false \
    processors_to_run=all \
    2>&1 | tee ${OUTPUT_DIR}/pipeline_full.log

# Calculate duration
END_TIME=$(date +"%Y-%m-%d %H:%M:%S")
END_EPOCH=$(date +%s)
DURATION=$((END_EPOCH - START_EPOCH))
HOURS=$((DURATION / 3600))
MINUTES=$(( (DURATION % 3600) / 60 ))

echo ""
echo "=========================================="
echo "Pipeline completed successfully!"
echo "=========================================="
echo "Start time: ${START_TIME}"
echo "End time: ${END_TIME}"
echo "Duration: ${HOURS}h ${MINUTES}m"
echo ""
echo "Results saved to: ${OUTPUT_DIR}"
echo ""

# Check final output
if [ -f "${OUTPUT_DIR}/de/manifest_32.json" ]; then
    FINAL_COUNT=$(grep -c '"audio_filepath"' "${OUTPUT_DIR}/de/manifest_32.json" 2>/dev/null || echo "0")
    echo "Final dataset: ${FINAL_COUNT} segments processed"

    # Check WebDataset output
    if [ -d "${OUTPUT_DIR}/de/webdataset" ]; then
        TAR_COUNT=$(ls ${OUTPUT_DIR}/de/webdataset/train/*.tar 2>/dev/null | wc -l)
        echo "WebDataset: ${TAR_COUNT} tar shards created"
    fi
else
    echo "WARNING: Final manifest not found!"
    echo "Check ${OUTPUT_DIR}/pipeline_full.log for errors"
fi

echo ""
echo "Pipeline Features Applied:"
echo "  ✓ Channel normalization (25% boost for quieter channel)"
echo "  ✓ Menu item spelling fixes (German)"
echo "  ✓ Whisper model error corrections"
echo "  ✓ German grammar and punctuation restoration"
echo "  ✓ Capitalization (including nouns)"
echo "  ✓ Filler word preservation"
echo ""
echo "Next Steps:"
echo "  1. Review the log: less ${OUTPUT_DIR}/pipeline_full.log"
echo "  2. Upload to HuggingFace: ./run/upload_webdataset_to_hf.sh ${OUTPUT_DIR}/de/webdataset VoxAI/${BRAND}-${COUNTRY}-${CUTOFF_DATE}"
echo "=========================================="
